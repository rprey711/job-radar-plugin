---
name: review
description: "Nach dem Versand: die tatsächlich gesendete Fassung im Bewerbungsordner ablegen oder zwei kurze Fragen dazu beantworten, daraus Lernnotizen für die nächsten Bewerbungen ziehen. Etwa 5 Minuten, Voraussetzung ist ein verschicktes Dokument."
disable-model-invocation: true
---

# /review

Ablauf „Review“ von Job Radar. Das Skript des Ablaufs liegt auf dem Server und kommt über den Connector; dieser Skill sagt nur, in welcher Reihenfolge es geholt wird und wo die Werkzeuge liegen.

## Reihenfolge

1. `job_radar_status` aufrufen. Fehlt das Werkzeug oder kommt ein Anmeldefehler: siehe „Verbindung fehlt“, dann abbrechen.
2. `.jobradar/ablauf_review.md` lesen, falls vorhanden; dort steht, wo der letzte Durchlauf stand.
3. `anleitung_laden(thema="review")` aufrufen und das Skript Schritt für Schritt abarbeiten. Nicht überfliegen, nichts überspringen, Checkpoints des Skripts einhalten.
...
5. Zwischenstand nach `.jobradar/ablauf_review.md`, wenn der Ablauf über mehrere Sitzungen geht; am Ende aufräumen.
